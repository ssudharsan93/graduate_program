...
def main():
    ...
    final_static_estimation, desirable_move, static_estimation_count = ABAlgorithmMidgameEndgame(bp, 0, depth)
...
#initiates the alpha beta pruning algorithm
def ABAlgorithmMidgameEndgame(bp, level, max_depth):
    pos_inf = sys.maxsize
...
def ABMaxMin(board_position, alpha, beta, level, max_depth):
    ...
    if ( level == max_depth ):
        ...
        L = GenerateMovesMidgameEndgame(board_position)
...
def ABMinMax(board_position, alpha, beta, level, max_depth):
    ...
    if ( level == max_depth ):
        ...
        L = GenerateMovesMidgameEndgameBlack(board_position)
