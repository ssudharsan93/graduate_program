...
def build_knowledge_base(corpus_dict, tf_idf_dict, keywords, keyword_mappings=None):
    knowledge_base_dict = dict()

    if ( keyword_mappings ):
        for elem in keyword_mappings:
            knowledge_base_dict[elem[1]] = corpus_dict[elem[0]]['sentences']
...
def make_corpus_dictionaries():
    ...
    for clean_corpus_file in clean_corpus_files:
        ...
        keyword_str = (list(filter(lambda x: x != '', content.split("\n")))[0]).split("|")[0]
...
def main():
    ...
    keyword_mappings = [ ( corp_file, corpus_dict[corp_file]['keyword']) for corp_file in corpus_dict.keys() ]

    knowledge_base = build_knowledge_base(corpus_dict, tf_idf_dict, keywords, keyword_mappings)
