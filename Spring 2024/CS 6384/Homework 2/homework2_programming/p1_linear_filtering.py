"""
CS 6384 Homework 2 Programming: Part 1&2 - mean and gaussian filters
...
def linear_local_filtering(
    ...
    for i in range(kernel_size // 2, sizeX - kernel_size // 2):
        for j in range(kernel_size // 2, sizeY - kernel_size // 2):
            ...
            window_width = kernel_size // 2
            i_dim_start = i - window_width
            i_dim_end = i + window_width
            j_dim_start = j - window_width
            j_dim_end = j + window_width

            curr_img_mat = np.array([ 
                [ img[i_itr][j_itr] for i_itr in range(i_dim_start, i_dim_end + 1)] 
                for j_itr in range(j_dim_start, j_dim_end + 1) 
            ])
            result_matrix = filter_weights * curr_img_mat
            pixel_val = np.sum(result_matrix)

            img_filtered[i, j] = pixel_val # todo: replace 0 with the correct updated pixel value
...
def gauss_kernel_generator(kernel_size: int, spatial_variance: float) -> np.ndarray:
    ...
    x_center = kernel_size // 2
    y_center = kernel_size // 2

    for x in range(kernel_size):
        for y in range(kernel_size):
            gaussian_val = ( 1.0  / ( 2 * np.pi * spatial_variance ) ) * np.exp( 
                -1.0 * ( ( pow( ( x - x_center ), 2 ) + pow( ( y - y_center ), 2 ) ) / ( 2 * spatial_variance ) )
            )
            kernel_weights[x][y] = gaussian_val
