...
def sift_matching(des1, des2):
    ...
    for des1_itr in range(m):
        L2_dist = np.array([0] * n)
        for des2_itr in range(n):
            L2_dist[des2_itr] = np.linalg.norm(des1[des1_itr] - des2[des2_itr])
            curr_index = np.where(L2_dist == np.min(L2_dist))
            index[des1_itr] = curr_index[0][0]
